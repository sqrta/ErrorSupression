...
def getLogicOp(getHtarBlock, blocksize, blockNum, Astring, debug=False):
    ...
    Heff_2nd_order = P0 @ Henc @ P0 - (
        P0 @ Henc @ Q0 @ HpenInverse @ Q0 @ Henc @ P0 / lambdaPen
    )
    ...
    def getEff(n, pauliList):
        ...
        Vs = [
            np.kron(ket2Vec(n // 4, Pstr[0]), ket2Vec(n // 4, Pstr[1])) / 2
            for Pstr in Pstrs
        ]
...
def searchOp(getHtarBlock, targetOp, blocksize, blockNum):
    pauliPairs = [f"X{i}*Z{j}" for i in range(3) for j in range(4, 8)] + [
        f"Z{i}*X{j}" for i in range(3) for j in range(4, 8)
    ]
...
def checkOps(Ops, Astring, lamb, blockNum):
    ...
    P0 = np.identity(Q0.shape[0]) - Q0
    Heff_2nd_order = P0 @ Henc @ P0 - (
        P0 @ Henc @ Q0 @ HpenInverse @ Q0 @ Henc @ P0 / lamb
    )
