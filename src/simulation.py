...
def EncodeTar(Htar, blockNum, lamb ,crossTerm):
    logDict = {}
...
def getU(blocksize, blockNum):
    v1 = ket2Vec(blocksize, ['0001', '-1110'])
    v2 = ket2Vec(blocksize, ['-0010', '1101'])
    v3 = ket2Vec(blocksize, ['0100', '-1011'])
    v4 = ket2Vec(blocksize, ['-0111', '1000'])
...
def getHencBlock(blockNum, lambdaPen, crossTerm):
    Htar_block = getHtarBlock(blockNum)
    Henc_block = EncodeTar(Htar_block, blockNum, lambdaPen, crossTerm)
    return Henc_block
...
def getError(lambdaPen, blockNum):
    ...
    for i in range(blockNum-1):
        ...
        # B2.append((1, f'Z{2*i+2}'))
        Htar_block.append((1, f'Z{2*i+1}*Z{2*i+2}'))
    crossTerm = [(1, f'Z{4*i+4}*Z{4*i+5}'), ((8*lamb/3)**0.5, f'Z{4*i+1}*X{4*i+7}+Z{4*i+3}*X{4*i+7}')]
    Henc_block = EncodeTar(Htar_block, blockNum, lambdaPen, crossTerm)
    U = getU(n, blockNum)
...
def sanityCheck(lambdaPen, blockNum):
    n = 4
    size = n*blockNum
    Hpen_block = []
    Htar_block = []
