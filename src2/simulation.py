...
def EncodeTar(Htar, blockNum, lamb):
    ...
    for i in range(blockNum):
        ...
        logDict[f"Z{2*i+1}*Z{2*i+2}"] = [
            (1, f"Z{4*i+4}*Z{4*i+5}"),
            ((8 * lamb / 3) ** 0.5, f"Z{4*i+1}*X{4*i+6}+Z{4*i+3}*X{4*i+6}"),
        ]
    res = []
    for term in Htar:
...
def getError(lambdaPen, blockNum, iters=1):
    ...
    for i in range(iters):
        ...
        V = (
            [PauliTerm(size, f"X{i}", epsilons[i]) for i in range(size)]
            + [PauliTerm(size, f"Z{i}", epsilons[i + size]) for i in range(size)]
            + [PauliTerm(size, f"Y{i}", epsilons[i + 2 * size]) for i in range(size)]
        )
