...
if __name__ == '__main__':
    ...
    Henc1 = sum([X(4*i)*X(4*i+1) - X(4*i)*X(4*i+2) for i in range(nb)]) # logical X
    Henc1 += sum([Z(4*i)*Z(4*i+1) + Z(4*i)*Z(4*i+2) for i in range(nb)]) # logical Z
    Henc1 += sum([Z(4*i+1)*Z(4*i+2) for i in range(nb)]) # inner-block logical ZZ
    Henc1 += sum([Z(4*i+4)*Z(4*i+5) for i in range(nb-1)]) # compensate for residual terms from cross-block gadget
    Henc1.L = n
    Henc2 = sum([np.sqrt(8/3) * (Z(4*i+1)*X(4*i+6) + Z(4*i+3)*X(4*i+6)) for i in range(nb-1)]) # cross-block logical ZZ
    Henc2.L = n
