...
if __name__ == '__main__':
    ...
    for c in range(n_col):
        for r in range(n_row):
            ...
            if r < n_row - 1: # vertical cross-block Ising interactions between block (c,r) and block (c,r+1)
                u1, u2, d1, d2 = lidx(c, r, 1), lidx(c, r, 2), lidx(c, r + 1, 1), lidx(c, r + 1, 2)
                Htar += Z(u1) * Z(d2) + Z(u2) * Z(d1)
    
            if c < n_col - 1: # horizontal cross-block Ising interactions between block (c,r) and block (c+1,r)
    ...
    for lamb in lamb_list:
        for seed in seed_list:
            for t in t_list:
                ...
                # output
                f = open(f"output_2dTFIM_{n_row}x{2*n_col}_noise={noise}_seed={seed}.txt", "a")
