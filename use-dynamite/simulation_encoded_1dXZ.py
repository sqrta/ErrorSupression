...
if __name__ == '__main__':
    ...
    Henc1 += sum([Z(4*i+4)*Z(4*i+5) + X(4*i)*X(4*i+1) for i in range(nb-1)]) # compensate for residual terms from cross-block gadget
    Henc1.L = n
    Henc2 = sum([np.sqrt(2/3) * (4*Z(4*i+1)*X(4*i+6) + Z(4*i+3)*X(4*i+6) + 3*Z(4*i+1)*X(4*i+4)) for i in range(nb-1)]) # cross-block logical ZZ & XX
    Henc2.L = n
    ...
    for lamb in lamb_list:
        for seed in seed_list:
            for t in t_list:
                ...
                # output
                f = open(f"output_1dXZ_{nb}blocks_noise={noise}.txt", "a")
